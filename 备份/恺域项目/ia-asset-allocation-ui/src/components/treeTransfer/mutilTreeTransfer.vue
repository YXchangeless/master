...
export default {
  ...
  methods: {
    // 移动到右边
    toRight() {
      const treeLeftSelectLeaf = this.$refs.treeLeft.getCheckedNodes().filter((item) => {
    ...
    // 移动到左边
    toLeft() {
      const treeRightSelect = this.$refs.treeRight.getCheckedNodes().filter((item) => {
        return item.children && item.children.length !== 0
      })
      const keys = this.$refs.treeRight.getCheckedKeys()
      this.$emit('update:leftData', JSON.parse(JSON.stringify([...this.leftData, ...treeRightSelect])))
      this.$refs.treeRight.setCheckedNodes([])
      this.$refs.treeLeft.setCheckedNodes([])
