  ...
  <div class="transfer" style="width:100%;display: flex;justify-content: center;align-content: center">
    ...
    <div class="modeSelect">
      <div class="titleBox">
        ...
        <!--        >指数资产树-->
        <!--        </el-checkbox>-->
        <span class="titleFont">指数资产树</span>
    ...
    <div class="penetrateTree">
      <div class="titleBox">
        ...
        <!--        >已选中的指数资产-->
        <!--        </el-checkbox>-->
        <span class="titleFont">已选中的指数资产</span>
...
export default {
  ...
  methods: {
    ...
    toRight() {
      ...
      this.$emit('update:rightData', JSON.parse(JSON.stringify([...treeLeftSelectLeaf])))
      this.removeSelect(this.leftData, keys)
      this.$refs.treeLeft.setCheckedNodes([])
      this.leftSelect = []
      this.$emit('applyIndexSelect', this.cacheLeft)
    },
    // 移动到左边
    toLeft() {
      this.$emit('applyIndexSelect', JSON.parse(JSON.stringify(this.rightData)), true)
      // this.$emit('applyIndexSelect', this.rightData, true)
    ...
    handleLeftAssetTreeCheckChange(obj, checked) {
      if (!obj.children) {
        // 如果没有子节点，为叶子节点，即指数节点
        if (checked) {
          this.$refs.treeLeft.setCheckedNodes([obj])
          this.cacheLeft = JSON.parse(JSON.stringify([obj]))
          // 如果选中状态 添加到右边
...
.transfer {
  ...
  .el-tree-node__content {
    ...
    .el-tree-node__label:hover {height: auto; word-break:break-all; white-space: pre-wrap;  text-decoration: none;}
  }
